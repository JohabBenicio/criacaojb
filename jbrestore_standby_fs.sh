...
#-- Descrição           : Atualização do standby automatico (file system)
#-- Nome do arquivo     : jbrestore_standby_fs.sh
#-- Data de criação     : 20/03/2015
#-- -----------------------------------------------------------------------------------



rm -f /tmp/jbrestore_standby_fs.sh

vi /tmp/jbrestore_standby_fs.sh
i
...
read -p "Informe o diretório onde será armazenado os datafiles: " DIRDB
...
#find $ORACLE_HOME/dbs/init$ORACLE_SID.ora -exec sed -i '/CONVERT/d' {} \; 
#find $ORACLE_HOME/dbs/init$ORACLE_SID.ora -exec sed -i '/convert/d' {} \; 
...
startup nomount;
alter system set  control_files = '$DIRDB/controlfile01.dbf' comment='Set by Johab' scope=spfile;
shutdown immediate;
startup nomount;
quit;
...
run{
   allocate channel c1 device type disk;
EOF
cat /tmp/datafiles.log | grep "\*\*\*" | awk '{print "   set newname for datafile " $1 " to '\'$DIRDB'/" $3 $1 ".dbf'\'';"}'>>/tmp/restore.rcv
cat /tmp/datafiles.log | grep "TEMP" | awk '{print "   set newname for tempfile " $1 " to '\'$DIRDB'/" $3 $1 ".dbf'\'';"}'>>/tmp/restore.rcv
cat <<EOF>>/tmp/restore.rcv
   $CATALOG_RMAN
   restore database;
   switch datafile all;
...
LOG=$(cat /tmp/jbrestore_standby.log | grep "ORA-\|RMAN-" | grep -v "RMAN-07517" | wc -l);
if [ "$LOG" -eq "0" ]; then echo -e "[    $MSG_SUCESSO    ]"; else echo -e "[   $MSG_FALHA  ]"; echo "Analise o log /tmp/jbrestore_standby.log";exit; fi
...
sh /tmp/jbrestore_standby_fs.sh
