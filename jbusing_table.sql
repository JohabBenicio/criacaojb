...
vtable varchar2(90):='&NOME_TABELA';
...
for x in (
   ...
   WHERE s.username is not null and l.session_id=s.sid and l.object_id=o.object_id and o.object_name=upper(vtable) and o.object_type='TABLE'
...
if vvalid = 0 then
  ...
  dbms_output.put_line('NESTE MOMENTO NAO HA USUARIOS USANDO A TABELA >>>>> '||vtable);
  dbms_output.put_line(JBQB);
end if;
