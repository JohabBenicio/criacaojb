...
BEGIN
	...
	FOR X IN (SELECT JOB,LOG_USER,TO_CHAR(LAST_DATE,'DD/MM/YYYY HH24:MI:SS') LAST_DATE,TO_CHAR(NEXT_DATE,'DD/MM/YYYY HH24:MI:SS') NEXT_DATE,WHAT,FAILURES,BROKEN,INTERVAL FROM DBA_JOBS ORDER BY BROKEN,JOB)  LOOP
		...
		DBMS_OUTPUT.PUT_LINE('INTERVALO DA EXECUCAO:...... '||X.INTERVAL||CHR(10));
		...
		IF X.BROKEN = 'N' THEN
			DBMS_OUTPUT.PUT_LINE('DESABILITAR JOB:............ EXEC DBMS_IJOB.BROKEN('||X.JOB||', TRUE);');
		ELSE
			DBMS_OUTPUT.PUT_LINE('HABILITAR JOB:.............. EXEC DBMS_IJOB.BROKEN('||X.JOB||', FALSE);');
		END IF;
