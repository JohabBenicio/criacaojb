...
cat $ODS_HOME/config/$DBN.conf | grep -i job_type | cut -d "." -f 1| while read dryjobs
do
cat <<EOF
############################################################################################################################
# DRYRUN do job de backup $DBN.$dryjobs
############################################################################################################################

EOF
$ODS_HOME/bin/orabkp backup -d $DBN -j $dryjobs -dryrun
echo -e "\n"
done
fi
echo -e "\n"
