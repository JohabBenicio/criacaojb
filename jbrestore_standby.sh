...
#-- Nome do arquivo     : jbrestore_standby.sh
#-- Data de criação     : 20/03/2015
#-- Data de atualização : 24/03/2015
#-- -----------------------------------------------------------------------------------
...
if [ ! -f "$CONTR_STANDBY" ]; then echo -e "\n\nEste arquivo não existe!\n\n"; exit; fi;
...
run{
	...
	restore controlfile to '$DIR/controlfile01.dbf' from '$CONTR_STANDBY';
	release channel c1;
}
EOF
