...
cat \$ANL_HOME/temp/listenerlog.log
...
cat <<EOF1>$ANL_HOME/sh/listener_log.sh

find \$ORACLE_BASE -name listener.log | while read LOG
do
echo -e "\n#-- -------------------------------------------------------------------------------------------------"
echo "#-- LOG LISTENER: (Alertas nas ultimas \$NUMTNS linhas) - \$ORACLE_SID"
echo "#-- -------------------------------------------------------------------------------------------------"
echo "#-- \$LOG"
echo "" 
tail -\$NUMTNS \$LOG 2>>/dev/null | grep -4i "TNS-\|ORA-" | while read TNS_LOG
do
echo \$TNS_LOG 
done
echo -e "\n\n\n=====================================================================================================\n"
...
\$ANL_HOME/sh/listener_log.sh > \$ANL_HOME/temp/listenerlog.log
