...
if [ -d "$ORACLE_BASE/admin/scripts" ]; then
    ...
    if [ -d "$ANL_BASE/analysis" ]; then tar -czf /tmp/bkp_jbanalisys_$(date +"%d%m%Y").tar.gz $ANL_BASE/analysis; rm -rf $ANL_BASE/analysis/*; mv /tmp/bkp_jbanalisys_$(date +"%d%m%Y").tar.gz $ANL_BASE/analysis; fi 2>>/dev/null
...
export NOME_LOG=Analise_$NCLIENTE_\$HOSTNAME_\`date +"%d"_"%b"_"%G"_"%H%M"\`.log
...
sh \$ANL_HOME/carrega_log.sh > \$LOG1
...
cat <<EOF>$ANL_HOME/carrega_log.sh
echo -e "\n============================================================================================" 
