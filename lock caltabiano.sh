...
xblock number;
BEGIN
select count(*) into xblock from v\$lock a, v\$lock b
where a.block > 0 and b.block = 0
...
date >> /tmp/lock_hunter.log

echo -e "\n" >> /tmp/lock_hunter.log

sqlplus -S "/ as sysdba" <<EOF>>/tmp/lock_hunter.log
...
BEGIN
    ...
    LOOP
        ...
        DBMS_OUTPUT.PUT_LINE('                        WAITING                                  BLOCKER');
        DBMS_OUTPUT.PUT_LINE('                        ---------------------------------------- ----------------------------------------');
