...
# Modo de conexão
#
export CONN_RAT="sqlplus -S sys/systemusd2003' as sysdba' "
...
TO=johab@teor.inf.br

sendEmail -f dbmonitor@teor.inf.br -t $TO,$CC -s smtp.teor.inf.br:587 -u "Analise ambiente_$HOSTNAME" -m "Segue em anexo a analise do ambiente." -a $LOG1 -xu "dbmonitor@teor.inf.br" -xp "ju5u6hxi"
...
for x in (
   SELECT s.sid, s.last_call_et, s.status, s.username, s.osuser, s.program,   s.sql_hash_value, s.inst_id, s.machine,o.object_name,DECODE ( l.locked_mode, 0, 'None', 1, 'NoLock', 2, 'Row-Share (SS)', 3, 'Row-Exclusive (SX)', 4, 'Share-Table', 5, 'Share-Row-Exclusive (SSX)', 6, 'Exclusive','[Nothing]')   LOCKED_MODE,o.owner,o.object_type,s.serial# serial
   ...
   group by s.sid, s.serial# , s.last_call_et, s.status, s.username, s.osuser, s.program,   s.sql_hash_value, s.inst_id, s.machine,o.object_name,l.locked_mode,o.owner,o.object_type
   ORDER BY s.last_call_et asc
)loop
