...
export NOME_LOG=Analise_$NCLIENTE_\$ORACLE_SID_\$HOSTNAME_\`date +"%d"_"%b"_"%G"_"%H%M"\`.log

if [ ! -d "\$ANL_HOME/log/\$ORACLE_SID" ]; then
  mkdir -p \$ANL_HOME/log/\$ORACLE_SID
fi

export LOG1=\$ANL_HOME/log/\$ORACLE_SID/\$NOME_LOG
export LOG2=\$ANL_HOME/log/\$ORACLE_SID/Analise_DBA_$NCLIENTE_\$ORACLE_SID_\$HOSTNAME_\`date +"%d"_"%b"_"%G"_"%H%M"\`.log
...
if [ "\$COMPACT" -gt "60" ]; then
  tar -cvzf \$ANL_HOME/log/\$ORACLE_SID/Analise_$NCLIENTE_\$ORACLE_SID_\`date +"%d"_"%b"_"%G"\`.tar.gz \$ANL_HOME/log/Analise*.log
...
sendEmail -f dbmonitor@teor.inf.br -t \$EMAIL_DBA -s smtp.teor.inf.br:587 -u "Analise NCLIENTE" -m "Segue em anexo a analise do ambiente." -a \$LOG1 -xu "dbmonitor@teor.inf.br" -xp "ju5u6hxi"

EOF1


cat <<EOF1>$ANL_HOME/sh/exec_oracle_sql.sh
...
pro ============================================================================================
pro ================================= DADOS DO BANCO E INSTANCIA ===============================
pro ============================================================================================
...
BEGIN
  ...
  IF v_value < 90 THEN
    DBMS_Output.Put_Line('Increase SHARED_POOL_SIZE parameter to bringvalue above 90%');
  ELSE
  ...
  IF v_value < 99 THEN
    DBMS_Output.Put_Line('Increase SHARED_POOL_SIZE parameter to bringvalue above 99%');
  ELSE
    DBMS_Output.Put_Line('Value Acceptable.');  
  END IF;
  ...
  IF v_value < 89 THEN
    DBMS_Output.Put_Line('Increase DB_BLOCK_BUFFERS parameter to bringvalue above 89%');
  ELSE
    DBMS_Output.Put_Line('Value Acceptable.');  
  END IF;
  ...
  IF v_value > 5 THEN
    DBMS_Output.Put_Line('Increase SORT_AREA_SIZE parameter to bringvalue below 5%');
  ELSE
    DBMS_Output.Put_Line('Value Acceptable.');  
  END IF;
...
pro "USUARIOS EXECUTANDO PROCESSOS NO BANCO DE DADOS COM MAIS DE 10 HORAS"
pro "===================================================================="
pro
...
pro "TABELAS SENDO USADAS (qualquer tipo de lock)"
pro "============================================"
pro
#-- ---------------------------------------------------------------------------------------------------------#
#-- Autor               : Johab Benicio de Oliveira.
...
pro
pro =================================================================================================
pro ======================= "ESTATISTICAS PARA O OTIMIZADOR DE CONSULTAS (CBO)" =======================
pro =================================================================================================
